...
namespace SubscriptionService.BusinessLogic.EventStoreContext
{
    public class EventStoreConnectionSettings
    {
        ...
        private void ParseConnectionString(String connectionString)
        {
            ...
            Int32 addressStart = connectionString.IndexOf("@");

            if (addressStart <= 0)
            {
                // Invalid Connection String, no address
                throw new ArgumentException("Invalid Connection String - no address seperator (@) present");
            }

            // Get the port seperator, this is the end of the address
            Int32 portStart = connectionString.IndexOf(":", addressStart);

            if (portStart <= 0)
            {
                // Invalid Connection String, no port
                throw new ArgumentException("Invalid Connection String - no port seperator (:) present");
            }

            Int32 portEnd = connectionString.IndexOf(";", portStart);

            if (portEnd <= 0)
            {
                // Invalid Connection String, invalid format
                throw new ArgumentException("Invalid Connection String - no section end seperator (;) present after the port");
            }
            
            // Get the port
            var portValue = connectionString.Substring(portStart + 1, (portEnd - portStart) - 1);

            // Check the seperator is a valid one, not another section seperator
            if (portValue.Length > 5)
            {
                // Invalid Connection String, invalid format
                throw new ArgumentException("Invalid Connection String - no valid section end seperator (;) present after the port");
            }
            ...
            Int32 userCredentialsStart = connectionString.IndexOf("//");

            if (userCredentialsStart <= 0)
            {
                // Invalid Connection String
                throw new ArgumentException("Invalid Connection String - no protocol/ user credentials seperator (//) present");
            }
